...
namespace PlusLocal
{
    internal class LocalAAXC
    {
        public static void GetPaths(string aax)
        {
            var aPath = Path.GetDirectoryName(aax);
            var aFile = Path.GetFileNameWithoutExtension(aax);

            var Voucher = Path.Combine(aPath, aFile + ".voucher");
            ...
            string filename = Other.SoftWare($@"{AppDomain.CurrentDomain.BaseDirectory}src\tools\mediainfo.exe", $"\"{aax}\" --Inform=General;%Album%", false);


            filename = filename.Trim().Replace(":", " -");

            aaxcFile.SetDecryptionKey(Settings.content_license.license_response.key, Settings.content_license.license_response.iv);

            aaxcFile.ConvertToMp4a(File.OpenWrite($@"{aPath}\{filename}.m4b"));
            aaxcFile.Close();
        }
    }
